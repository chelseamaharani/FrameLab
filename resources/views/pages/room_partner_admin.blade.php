...
@section('content')
	...
	<div class="bg-primary text-secondary rounded-3xl p-6 h-[100vh] max-h-[485px]">
		...
		<div class="overflow-x-auto overflow-y-auto w-full max-h-[370px]">
          	<table class="table-fixed w-full border-collapse border border-gray-300 text-black shadow-sm rounded">
            	<thead>
            	...
            	<tbody class="bg-white">
					@foreach ($data as $index => $item)
						<tr class="hover:bg-gray-50">
							...
							<td class="px-2 py-2 border text-center w-48 break-words whitespace-normal">{{ $item->noTelepon }}</td>
							<td class="px-2 py-2 border text-center w-20">{{ $item->studio_type }}</td>
							<td class="border px-2">
								<div class="flex justify-center gap-1">
									<button type="button"
										onclick="openEditModal({{ $item->id }}, `{{ $item->room_name }}`, `{{ $item->description1 }}`, `{{ $item->description2 }}`, `{{ $item->description3 }}`, `{{ $item->noTelepon }}`, `{{ $item->studio_type }}`, `{{ $item->photo }}`)"
			...
			<script>
				function openEditModal(id, name, desc1, desc2, desc3, phone, type, photo) {
